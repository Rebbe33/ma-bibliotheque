...
// ── WishForm HORS de WishlistContent — évite le re-render qui ferme le clavier ──
function WishForm({ form, setForm, onSave, onCancel }: {
  form: WForm
  setForm: React.Dispatch<React.SetStateAction<WForm>>
  onSave: () => void
  onCancel: () => void
}) {
  return (
...
function WishlistContent() {
  ...
  function closeSheet() {
    setShowAdd(false); setEditWish(null); setForm(EMPTY); setStep('search')
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="-mt-2 flex gap-2 overflow-x-auto py-1 -mx-4 px-4">
        ...
        {noLocationCount > 0 && (
          <button onClick={() => setNoLocationFilter(!noLocationFilter)}
            className={`flex-shrink-0 px-4 py-2 rounded-pill font-black text-sm transition-all whitespace-nowrap ${
              noLocationFilter ? 'bg-amber-light text-amber-dark shadow-card ring-2 ring-offset-1 ring-current' : 'bg-white text-gray-500 shadow-sm'
            }`}>
                ...
                <div className="flex-1 p-3.5 min-w-0">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {w.where_to_find
                        ? <div className="flex items-center gap-1 mt-1 text-[11px] text-gray-400 font-semibold"><MapPin size={10}/>{w.where_to_find}</div>
                        : <div className="flex items-center gap-1 mt-1 text-[11px] text-amber-dark font-semibold"><MapPin size={10}/>Non renseigné</div>
                      }
                    </div>
                  </div>
                </div>
      ...
      <BottomSheet
        open={showAdd || !!editWish}
        onClose={closeSheet}
        title={editWish ? 'Modifier le souhait' : 'Ajouter un souhait ✨'}>
        {step === 'search' && !editWish ? (
          <div>
            <BookSearch onSelect={fromGoogle} onManual={() => setStep('form')} mode="wishlist"/>
            <button onClick={closeSheet} className="mt-2 btn btn-ghost w-full">Annuler</button>
          </div>
        ) : (
          <WishForm
            form={form}
