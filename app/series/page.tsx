...
interface GoogleBookWithCheck extends GoogleBook {
  checked: boolean
}
...
function SeriesContent() {
  ...
  const [searchResults, setSearchResults] = useState<GoogleBookWithCheck[]>([])
  ...
  async function searchSeries() {
    ...
    try {
      ...
      const withSeries = data
        .filter((b: GoogleBook) => b.seriesInfo?.bookDisplayNumber || b.title.match(/#\d+|tome|vol\.|t\.\d/i))
        .map((b: GoogleBook) => ({ ...b, checked: true }))
      setSearchResults(withSeries.length > 0 ? withSeries : data.map((b: GoogleBook) => ({ ...b, checked: true })))
    } catch { setSearchResults([]) }
    setSearching(false)
  }

  function toggleCheck(id: string) {
    setSearchResults(r => r.map(b => b.id === id ? { ...b, checked: !b.checked } : b))
  }

  function toggleAll() {
    const allChecked = searchResults.every(b => b.checked)
    setSearchResults(r => r.map(b => ({ ...b, checked: !allChecked })))
  }

  async function addSelected() {
    const selected = searchResults.filter(b => b.checked)
    ...
    for (const book of selected) {
      await supabase.from('books').insert({
        ...
        series_name: book.seriesInfo?.shortSeriesBookTitle || searchQuery,
  ...
  const selectedCount = searchResults.filter(b => b.checked).length
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4 bg-gradient-to-br from-violet-light to-pink-light space-y-3">
        <p className="font-black text-sm text-ink">🔍 Chercher une série sur Google Books</p>
        ...
        {!searching && searched && searchResults.length === 0 && (
          <div className="text-center py-4">
            <p className="font-black text-sm text-ink">😕 Aucun résultat</p>
            <p className="text-xs text-gray-500 mt-1">Essayez un autre titre de série</p>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <button onClick={toggleAll}
                ...
                {searchResults.every(b => b.checked) ? 'Tout décocher' : 'Tout cocher'}
              </button>
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {series.length === 0 ? 'Cherchez une série ci-dessus pour l\'ajouter !' : 'Essayez un autre terme'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
